...
def Decrypt(**kwargs):
    ...
    **kwargs=(vendor="cisco","juniper", hashed = Hashed password)
    ...
    if not kwargs.get("hashed"):
        return("")
    if "$9$" not in kwargs.get("hashed") and kwargs.get("vendor")=='juniper' or kwargs.get("hashed")=="" and kwargs.get("vendor")=='cisco':
        return("")
...
def decrypt_config(**kwargs):
    ...
    **kwargs=dict(
    ...
    if kwargs.get("vendor")=="cisco":
        hash_list=re.findall(r'(?:(?:key|password) 7|-key \S+ md5) (\S+)',config_string)
        ...
        hash_list=re.findall(r'.*(\$9\$\S+)\\";.*',config_string)
        clean_config=config_string
        for h in hash_list:
            replacement=hashing(h)
            clean_config=clean_config.replace(h,replacement) 
